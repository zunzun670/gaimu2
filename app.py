import streamlit as st
import random

# CSSでフォントサイズと太さを統一、さらに可愛く装飾
st.markdown(""" 
    <style> 
    .question-text, .stRadio label { 
        font-size: 18px !important; 
        line-height: 1.3 !important;
        font-weight: bold;
    }
    div[role="radiogroup"] > label {
        line-height: 1.2 !important;
        margin-bottom: 0.2rem !important;
    }
    /* サイドバーや隅に置くキャラクター用 */
    .daruma-text {
        font-size: 50px;
        text-align: center;
    }
    </style> 
""", unsafe_allow_html=True)

# 問題データ
questions = [
    {
        "question": "株式は元本保証されている？",
        "options": ["◯", "✕"],
        "answer": "✕",
        "explanation": "株式は価格が変動するから、元本保証はされていないよ",
        "source": "問題：二種外務員",
    },
    {
        "question": "投資信託の運用会社の役割は？",
        "options": ["資産の保管", "運用の指図", "販売", "監査"],
        "answer": "運用の指図",
        "explanation": "運用会社は投資信託の資産をどのように運用するかを決める役割を担っているよ",
        "source": "問題：二種外務員",
    },
    {
        "question": "債券の利子は変動する？",
        "options": ["変動する", "固定されている"],
        "answer": "固定されている",
        "explanation": "一般的な債券は固定利率で発行されいるよ",
        "source": "問題：二種外務員",
    },
    {
        "question": "株式会社が新しく株を発行して、投資家からお金を集める場所のことを流通市場という。",
        "options": ["◯", "✕"],
        "answer": "✕",
        "explanation": "新しく株を発行してお金を集めるのは発行市場。  \n流通市場は、すでに誰かが持っている株を売り買いする場所（証券取引所など）だよ",
        "source": "問題：二種外務員",        
    },
    {
        "question": "上場株式を売買するとき、株価がいくらであっても証券会社が受け取る売買委託手数料の金額は、国によって一律に決められている。",
        "options": ["◯", "✕"],
        "answer": "✕",
        "explanation": "それぞれの証券会社が自分たちで決めていいんだ。だからインターネットの証券会社みたいに「うちは手数料がすごく安いよ！」と競争できるようになったんだね",
        "source": "問題：二種外務員",        
    },
    {
        "question": "会社が倒産したとき、その会社の債券を持っている人は、株式を持っている人よりも優先的に、残ったお金を返してもらう権利がある。",
        "options": ["◯", "✕"],
        "answer": "◯",
        "explanation": "優先順位は1️⃣債権を持っている人（お金を貸している人）2️⃣株式を持っている人（会社のオーナー）。債権者に全部返した後余っていたら、株式を持っている人にももらえるよ",
        "source": "問題：二種外務員",
    },
    {
        "question": "上場会社が、すでに発行している株式をさらに新しく発行して、広く投資家からお金を集めることを「公募増資（こうぼぞうし）」という。",
        "options": ["◯", "✕"],
        "answer": "◯",
        "explanation": "会社はこれでお金を手に入れて、新しい工場を建てたり事業を広げたりするんだ",
        "source": "問題：二種外務員",
    },
    {
        "question": "証券会社が、客から預かっているお金や株を、証券会社自身の財産と混ざらないように分けて管理することを「分別管理（ぶんべつかんり）」という。",
        "options": ["◯", "✕"],
        "answer": "◯",
        "explanation": "万が一、証券会社がつぶれてもお客さんのお金が守られるための大事なルールだよ",
        "source": "問題：二種外務員",
    },
    {
        "question": "次のうち、債券の「格付け」において、一般的に「投資適格（投資しても比較的安全）」とされる最低ラインの格付けはどれか。",
        "options": ["AA", "A","BBB","BB"],
        "answer": "BBB",
        "explanation": "格付けは一般的に「BBB」以上が投資適格、ギリギリ合格ラインと覚えてね。「BB」以下が投機的（リスクが高い）とされているよ",
        "source": "問題：二種外務員",
    },
    {
        "question": "投資家が、証券会社に対して株式の売買を注文する際、価格を指定せずに「その時の市場価格で成立させてほしい」と依頼する方法を「指値注文」という。",
        "options": ["◯", "✕"],
        "answer": "✕",
        "explanation": "価格を指定しない注文は「成行（なりゆき）注文」だよ。指値は「1000円で買って」と値段を指し示す注文のことだね",
        "source": "問題：二種外務員",
    },
    {
        "question": "株式の売買取引において、最も優先して成立する注文は次のうちどれか。",
        "options": ["1,000円の指値買い注文", "成行買い注文","900円の指値買い注文","前日に出された1,050円の指値買い注文"],
        "answer": "成行買い注文",
        "explanation": "取引所では「価格優先の原則」があるよ。価格を指定しない成行注文は、どんな価格の指値注文よりも優先して成立するルールなんだ。",
        "source": "問題：二種外務員",
    },
    {
        "question": "生命保険の保険金は、相続税の課税対象にはならない。",
        "options": ["◯", "✕"],
        "answer": "✕",
        "explanation": "生命保険金は「みなし相続財産」として相続税の課税対象になるよ。  \nただし「500万円 × 法定相続人の数」まで非課税になる特例がある。たとえば法定相続人が3人なら、1,500万円まで非課税だよ。",
        "source": "問題：日本FP協会 3級ファイナンシャル・プランニング技能検定学科試験 2025年5月公表分",
    },
    {
        "question": "次のうち、所得税の課税対象とならないものはどれ？",
        "options": ["給与所得", "退職所得","宝くじの当せん金","不動産の賃貸収入"],
        "answer": "宝くじの当せん金",
        "explanation": "「非課税所得」として法律で定められているよ。ただし、懸賞金や競馬の払戻金などは課税対象になることがあるので注意！",
        "source": "問題：日本FP協会 3級ファイナンシャル・プランニング技能検定学科試験 2025年5月公表分",
    },
    {
        "question": "税理士の登録を受けていないファイナンシャル・プランナーが、顧客のために反復継続して確定申告書を作成しても、その行為が無償であれば税理士法に抵触しない。 ",
        "options": ["◯", "✕"],
        "answer": "✕",
        "explanation": "反復継続して他人の確定申告書を作成する行為は、税理士法に違反しちゃう。税務書類の作成は原則として税理士の独占業務。FPが行えるのは、あくまで一般的なアドバイスまで！",
        "source": "問題：日本FP協会 3級ファイナンシャル・プランニング技能検定学科試験 2025年5月公表分",
    },
    {
        "question": "雇用保険の高年齢雇用継続基本給付金は、原則として、60歳以上65歳未満の被保険者が、60歳到達時点に比べて賃金月額が85％未満に低下した状態で就労している場合に支給される。",
        "options": ["◯", "✕"],
        "answer": "✕",
        "explanation": "支給の条件は、賃金が60歳到達時点の75％未満に低下した場合。「75％未満」が正しいラインなので注意！",
        "source": "問題：日本FP協会 3級ファイナンシャル・プランニング技能検定学科試験 2025年5月公表分",
    },
    {
        "question": "国民年金の第1号被保険者の収入により生計を維持している配偶者で、20歳以上60歳未満の者は、国民年金の第3号被保険者となる。",
        "options": ["◯", "✕"],
        "answer": "✕",
        "explanation": "第3号になれるのは、第2号（会社員や公務員）に扶養されている配偶者だけ。第1号（自営業など）に扶養されている配偶者は、自分で保険料を払う第1号被保険者になるよ。",
        "source": "問題：日本FP協会 3級ファイナンシャル・プランニング技能検定学科試験 2025年5月公表分",
    },
    {
        "question": "国民年金の第3号被保険者は、確定拠出年金の個人型年金に加入することができる。",
        "options": ["◯", "✕"],
        "answer": "◯",
        "explanation": "2017年から第3号もiDeCo（個人型確定拠出年金）に加入OK！ただし、掛金の上限や手続きには注意が必要だよ。",
        "source": "問題：日本FP協会 3級ファイナンシャル・プランニング技能検定学科試験 2025年5月公表分",
    },
    {
        "question": "小規模企業共済の掛金月額は、1,000円から7万円までの範囲内で500円単位で選択することができる。",
        "options": ["◯", "✕"],
        "answer": "◯",
        "explanation": "途中で増減もできるから、事業の状況に合わせて調整できるのが魅力！",
        "source": "問題：日本FP協会 3級ファイナンシャル・プランニング技能検定学科試験 2025年5月公表分",
    },
    {
        "question": "契約転換制度を利用して、現在契約している生命保険を新たな契約に転換する場合、転換後契約の保険料は、転換前契約の加入時の年齢に応じた保険料率により算出される。",
        "options": ["◯", "✕"],
        "answer": "✕",
        "explanation": "転換後の保険料は、転換時の年齢に応じた保険料率で計算されるよ（＝年齢が上がるほど保険料も高くなるので転換のタイミングには注意）  \n🔍️契約転換制度（契約転換）とは現在加入している生命保険の積立部分（責任準備金）や配当金を「下取り（転換価格）」として、新しい保険へ乗り換える制度。古い契約の解約返戻金等が新しい保険に充当される",
        "source": "問題：日本FP協会 3級ファイナンシャル・プランニング技能検定学科試験 2025年5月公表分",
    },
    {
        "question": "乗換え勧誘において、顧客が現在持っている投資信託を解約させて、それと実質的に同じような別の投資信託を販売する場合、重要事項を説明せずに勧誘することは禁止されている。",
        "options": ["◯", "✕"],
        "answer": "◯",
        "explanation": "新しいものを売るために今のを解約させる「乗換え」は、手数料負担などでお客さんが損をすることがあるよ。だから、ちゃんと不利益になる可能性も説明しないといけないんだね。",
        "source": "問題：二種外務員（外務員の職務）",
    },
   {
        "question": "額面100円につき買付価格98円、表面利率（クーポン）年2％、残存期間5年の利付債券を新規発行時に購入した場合の「応募者利回り」はいくらか？",
        "options": ["2.00％", "2.04％", "2.40％", "2.45％"],
        "answer": "2.40％",
        "explanation": "計算式は：[2 + (100 - 98) / 5] / 98 × 100 = 2.448... だけど、外務員試験では小数点第3位以下を切り捨てることが多いよ。ここでは2.40%が正解。  \n🔍️計算問題は落ち着いて解こう！",
        "source": "問題：二種外務員（債券業務）",
    },
    {
        "question": "信用取引において、顧客が証券会社に預けなければならない委託保証金の額は、売買代金の30％以上、かつ、最低30万円以上でなければならない。",
        "options": ["◯", "✕"],
        "answer": "◯",
        "explanation": "「30%・30万円・原則6ヶ月が返済期限（制度信用取引の場合）」は信用取引の超重要ルール！レバレッジをかけて取引するから、一定以上の担保が必要なんだよ。  \n🔍️信用取引とは：証券会社からお金や株を借りて、自分の持っている資金以上の取引をすること",
        "source": "問題：二種外務員（株式業務）",
    },
    {
        "question": "投資信託の「クローズド・期間」とは、信託期間の全期間を通じて、いつでも解約請求ができる期間のことをいう。",
        "options": ["◯", "✕"],
        "answer": "✕",
        "explanation": "クローズド（閉じている）期間は、原則として「解約ができない」期間のこと。運用を安定させるために設けられているよ。",
        "source": "問題：二種外務員（投資信託）",
    },
    {
        "question": "日本証券業協会の「協会員の従業員に関する規則」において、外務員が顧客から売買の注文を受ける際、売買の価格を決めずに注文を受ける「成行注文」は禁止されている。",
        "options": ["◯", "✕"],
        "answer": "✕",
        "explanation": "成行注文は一般的な注文方法だよ。禁止されているのは「価格を勝手に決めて取引すること（ほしいままの価格による取引）」であって、成行注文そのものはOKなんだ！",
        "source": "問題：二種外務員（外務員の職務）",
    },
    {
        "question": "投資信託において、運用会社（委託者）からの指図を受けて、信託財産の保管や管理を実際に行う機関はどこか？",
        "options": ["証券会社（販売会社）", "受託銀行（信託銀行）", "投資信託協会", "日本証券決済機構"],
        "answer": "受託銀行（信託銀行）",
        "explanation": "お金や株を実際に金庫で預かるのは信託銀行（受託者）の役割。  \n🔍️運用会社（指図する人）、販売会社（窓口の人）との役割の違いを覚えよう！",
        "source": "問題：二種外務員（投資信託）",
    },
    {
        "question": "公社債の利回りを計算する際、額面100円あたりの「表面利率（クーポン） ＋ （売却価格 － 購入価格） ÷ 所有期間」という式で算出される利回りを何と呼ぶか？",
        "options": ["直接利回り", "最終利回り", "所有期間利回り", "応募者利回り"],
        "answer": "所有期間利回り",
        "explanation": "途中で買って、償還前に途中で売った場合の利回りだから「所有期間」利回りと言うよ",
        "source": "問題：二種外務員（債券業務）",
    },
    {
        "question": "信用取引において、株価の変動によって委託保証金の額が最低維持率を下回った場合、追加で差し入れなければならない保証金のことを何と呼ぶか？",
        "options": ["代用有価証券", "追証", "プレミアム", "証拠金"],
        "answer": "追証",
        "explanation": "「追加保証金」を略して「追証」だよ。これが発生すると、期限までにお金や株を追加で入れなきゃいけないから、投資家にとってはピンチのサインなんだ。",
        "source": "問題：二種外務員（株式業務）",
    },
    {
        "question": "日本証券業協会の定めに従い、外務員が顧客を訪問する際、必ず携帯し、提示を求められたら見せなければならないものは何か？",
        "options": ["住民票", "外務員証", "証券外務員合格証書", "会社の名刺"],
        "answer": "外務員証",
        "explanation": "外務員証は携帯必須！",
        "source": "問題：二種外務員（外務員の職務）",
    },
    {
        "question": "株式会社の貸借対照表（B/S）において、資産から負債を差し引いた純粋な資産のことを「純資産」と呼ぶ。",
        "options": ["◯", "✕"],
        "answer": "◯",
        "explanation": "資産（持っているもの）− 負債（借金）＝ 純資産（自分のもの）。この基本の式は、財務諸表の分析でとっても大事だよ！",
        "source": "問題：二種外務員（株式会社概論）",
        {
        "question": "将来の特定の日に、特定の価格で商品を「買う権利」を売買する取引のことを何と呼ぶか？",
        "options": ["先物取引", "コール・オプションの売買", "プット・オプションの売買", "スワップ取引"],
        "answer": "コール・オプションの売買",
        "explanation": "買う権利は「コール」、売る権利は「プット」だよ。",
        "source": "問題：二種外務員（デリバティブ取引）",
         {
        "question": "上場株式の配当金を受け取るためには、配当の権利を得るための「権利付最終日」にその株式を保有していなければならない。",
        "options": ["◯", "✕"],
        "answer": "◯",
        "explanation": "権利付最終日に株を持っていれば、翌日に売っても配当はもらえるよ。",
        "source": "問題：二種外務員（株式業務）",
         {
        "question": "投資信託の基準価額が10,000円のときに100万口購入し、その後基準価額が11,000円になったときにすべて解約した場合の利益（税金は考慮しない）はいくらか？",
        "options": ["1,000円", "10,000円", "100,000円", "1,100,000円"],
        "answer": "100,000円",
        "explanation": "計算：(11,000 - 10,000) × (1,000,000 / 10,000) = 100,000円。投資信託は1万口あたりの価額で表示されることが多いから、単位に気をつけて計算しよう！",
        "source": "問題：二種外務員（投資信託）",
    }
]

# --- 3. セッション状態の初期化（エラー回避のため一番上に！） ---
if "total_correct" not in st.session_state:
    st.session_state.total_correct = 0

if "current_q" not in st.session_state: 
    st.session_state.current_q = random.choice(questions) 
    st.session_state.answered = False 
    st.session_state.feedback = "" 
    st.session_state.explanation = ""

# 現在の問題を短い変数名に
q = st.session_state.current_q

# --- 4. サイドバーの表示 ---
with st.sidebar:
    st.write("### 合格祈願！進化するだるま")
    # 正解数（total_correct）に応じてだるまが進化
    if st.session_state.total_correct == 0:
        daruma = "⚪️"
    elif st.session_state.total_correct < 5:
        daruma = "🔴"
    elif st.session_state.total_correct < 15:
        daruma = "🏵️🔴🏵️"
    else:
        daruma = "✨👑🔴👑✨"
    
    st.markdown(f"<div class='daruma-text'>{daruma}</div>", unsafe_allow_html=True)
    st.write(f"これまでの正解数: {st.session_state.total_correct} 問")

# --- 5. メイン画面の表示 ---
st.title("二種外務員とFP3級 問題")

# 出典に基づいてアイコンを決定
icon = "💼" if "外務員" in q["source"] else "🏠"

st.markdown("<br>", unsafe_allow_html=True)
if "source" in q: 
    st.caption(f"{icon} {q['source']}")

st.markdown(f"<div class='question-text'>{q['question']}</div>", unsafe_allow_html=True) 
user_answer = st.radio(
    "", 
    q["options"], 
    key=q["question"], 
    label_visibility="collapsed"
)

# --- 6. 答え合わせロジック ---
if not st.session_state.answered:
    if st.button("答え合わせ 🔍"):
        st.session_state.answered = True
        if user_answer == q["answer"]:
            st.session_state.feedback = "✅ 正解！すごい！"
            st.session_state.total_correct += 1 
            st.balloons()
        else:
            st.session_state.feedback = "❌ 不正解！どんまい！"
        st.session_state.explanation = f"💡 解説：{q['explanation']}"
        st.rerun() # 結果を即座に反映させるため

# --- 7. フィードバックと解説 ---
if st.session_state.answered:
    st.divider() 
    st.markdown(f"### {st.session_state.feedback}")
    st.info(st.session_state.explanation)

    if st.button("次へ 🐾"):
        st.session_state.current_q = random.choice(questions)
        st.session_state.answered = False
        st.session_state.feedback = ""
        st.session_state.explanation = ""
        st.rerun()
